...
sub new {
	...
	my ($_trait, $package);
	if (ref($_package_or_trait) && $_package_or_trait->isa("Class::Trait::Config")) {
...
This class is to be used by other applications as a foundation from which to build various trait compostion tools. It attempts to decouple others from the internal representation of traits (currently the Class::Trait::Config object) and provide a more abstract view of them.
...
If given either a; I<Class::Trait::Config object>, I<a blessed perl object which uses traits> or I<a valid perl package name which uses traits>. This constructor will return a valid Class::Trait::Reflection object which can be used to examine the specific properties of the traits utilized.
...
Returns an array (or an array ref in scalar context) of method labels and operators which represent the requirements for the given trait.
...
Returns an array (or an array ref in scalar context) of method labels and operators which are in conflict for a given trait. This is only applicable for COMPOSITE traits.
...
=head2 utilitiy methods

=over 4

=item B<loadSubTraitByName>

Given a trait name, this method attempts to load the trait using the Class::Trait module, and then uses that trait to construct a new Class::Trait::Reflection object to examine the trait with. This can function both as a class method and an instance method. 
...
Given a subroutine reference, this method can be used to traverse a trait heirarchy. The subroutine reference needs to take two parameters, the first is the current trait being examined, the second is the number representing the depth within the trait heirarchy. 
